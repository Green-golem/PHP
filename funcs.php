...
{
    $resp = $client->get($url);
    return $html = $resp->getBody()->getContents();
}
...
function get_products(\DiDom\Document $document, \GuzzleHttp\Client $client): array
{
    ...
    $products = $document->find('article.card');
      foreach ($products as $product) {
          ...
          $url = $product->first('h2.card-title a')->attr('href');
...
function get_product(\DiDom\Document $document, \GuzzleHttp\Client $client, $url, $product_cnt): array
{
    ...
    $product['desc'] = $document->first('#productView-descriptions #tab-description')->innerHtml();

    //Image
    $image_path = $document->first('div.productView-img-container img.productView-image--default')->attr('src');
    $product['image'] = $product_cnt . "." . get_ext($image_path);

    if (!is_dir("images/{$page_title}")) {
        mkdir("images/{$page_title}");
    }
    file_put_contents("images/{$page_title}/{$product['image']}",
